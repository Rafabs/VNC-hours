...
class CSVLoader {
    ...
    // Determinar qual arquivo CSV carregar baseado no dia da semana
    getCSVFileForToday() {
        const today = new Date();
        const dayOfWeek = today.getDay(); // 0 = Domingo, 1 = Segunda, ..., 6 = Sábado
        
        if (dayOfWeek === 0 || this.isHoliday(today)) {
            return 'data/VNC_horario_dom_e_fer.csv';
        } else if (dayOfWeek === 6) {
            return 'data/VNC_horario_sab.csv';
        } else {
            return 'data/VNC_horario_seg_sex.csv';
        }
    }

    // Obter arquivo CSV baseado na seleção
    ...
    async loadCSVData(fileSelection = null) {
        ...
        try {
            ...
            console.log(`Dados carregados de ${csvFile}:`, this.data.length, "registros");
            return this.data;
        } catch (error) {
    ...
    parseCSV(csvText) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (values.length === headers.length) {
                ...
                console.warn(`Linha ${i + 1} ignorada - número de colunas incorreto:`, values);
            }
        }
